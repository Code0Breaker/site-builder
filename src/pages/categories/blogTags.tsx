...
const BlogTags = () =>{
    ...
    const edit = async () => {
        try {
            const data = await editBlogTag({name:name, _method:'put'}, selectedTag as number)
        if(data.success === true){
            window.location.reload()
        }
        } catch (error) {
        ...
        <FlexColumn>
                    ...
                    >
                      ...
                      <DialogContent sx={{display:'flex',gap:3}}>
                        <OutlinedInput placeholder="Enter blog tag name" value={name} onChange={e=>setName(e.target.value)}/>
                      </DialogContent>
                      <DialogActions>
