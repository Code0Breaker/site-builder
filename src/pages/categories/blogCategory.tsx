...
import { createBlogCategory, editBlogCategory, getBlogCategory, removeBlogCategory } from "../../api/categoryApi"
...
const BlogCategories = () =>{
    ...
    const [categoryName, setCategoryName] = useState({name:'', description:''})
        ...
        <FlexColumn>
                    ...
                    >
                      ...
                      <FlexCenter>
                         <label htmlFor="upload-flag">
                             ...
                             <input onChange={(e: React.ChangeEvent<HTMLInputElement>)=>e?.target?.files&&setImage(e.target.files[0])} type='file' accept="image/png, image/gif, image/jpeg" hidden id="upload-flag"/>
                           </label>
                         </FlexCenter>
                      <DialogContent sx={{display:'flex',gap:3}}>
                        <OutlinedInput placeholder="Enter category name" value={categoryName.name} onChange={e=>setCategoryName({...categoryName,name:e.target.value})}/>
                        <OutlinedInput placeholder="Enter category description" value={categoryName.description} onChange={e=>setCategoryName({...categoryName,description:e.target.value})}/>
                      </DialogContent>
                      <DialogActions>
            ...
            <PaperBox>
                <Flex flexWrap={'wrap'} width={'100%'} gap={3}>
                {
                    categories?.map(item=>{
                        return(
                            <FlexAlignCenter maxWidth={300} height={90} sx={{cursor:'pointer'}}>
                                <FlexColumn>
